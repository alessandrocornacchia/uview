...
def stream_traces(f, out, compression=True, debug=False):
    ...
    try:
        ...
        print('No existing trace IDs found, starting new collection')
        pass
...
def make_path(start_time, end_time, rootdir='datasets', append='', service=None):
    """
    Make path to a file
    """
    timerange = start_time.strftime("%Y%m%d%H%M") + '-' + end_time.strftime("%Y%m%d%H%M")
    directory = os.path.join(rootdir, timerange)
    if service:
