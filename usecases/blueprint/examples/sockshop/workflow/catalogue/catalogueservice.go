...
func (s *catalogueImpl) List(ctx context.Context, tags []string, order string, pageNum int, pageSize int) ([]Sock, error) {
	...
	slog.Info(fmt.Sprintf("Found %v socks matching tags %v", len(socks), tags))
	slog.Info(fmt.Sprintf("Returning page %v of size %v", pageNum, pageSize))
...
func (s *catalogueImpl) AddSock(ctx context.Context, sock Sock) (string, error) {
	// Delete any existing sock with this ID, or assing unique ID if not existing.
	if sock.ID != "" {
