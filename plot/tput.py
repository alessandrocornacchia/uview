...
# TODO this should be taken from environment variables. File should be 
# renamed to something like preprocessing.py
# Configuration
REMOTE_HOST = "ubuntu@192.168.100.2"
REMOTE_PATH = "~/cornaca/microview-cp/results/"
LOCAL_PATH = "./results/IPU"
VALID_ALGOS = ["FD", "VAE", "TH"]

def download_results(force=False):
    """Download results from remote machine using scp"""
    print("Downloading results from remote machine...")
    
    # Create local directory if it doesn't exist
    os.makedirs(LOCAL_PATH, exist_ok=True)
    
    # First, check what directories exist on the remote machine
    # TODO note the wildcard pattern  !!!!!!
    cmd = f"ssh {REMOTE_HOST} 'ls -d {REMOTE_PATH}metric_*_* {REMOTE_PATH}pods_*_* 2>/dev/null || echo \"No results found\"'"
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    
    if "No results found" in result.stdout:
        print("No results found on remote machine")
        return False
...
def collect_results():
    """Collect results from all wrk_output.txt files"""
    metrics_results = []
    pods_results = []
    
    # Process metrics results
    print("Collecting results from metrics...", os.listdir(LOCAL_PATH))
    ...
    # Process pods results
    print("Collecting results from pods...", os.listdir(LOCAL_PATH))
    for dir_name in os.listdir(LOCAL_PATH):
    ...
    # Process read_loop metrics files
    print("Collecting read_loop results for metrics...")
    for dir_name in os.listdir(LOCAL_PATH):
    ...
    # Process read_loop pods files
    print("Collecting read_loop results for pods...")
    for dir_name in os.listdir(LOCAL_PATH):
